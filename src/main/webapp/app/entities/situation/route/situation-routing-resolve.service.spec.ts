...
import situationResolve from './situation-routing-resolve.service';
...
describe('Situation routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ISituation returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          situationResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<ISituation>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
